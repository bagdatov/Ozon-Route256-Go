...
// JaroDistance is measuring how strings are similar to each other.
// From 0 to 1.
func JaroDistance(answer, guess string) float64 {
	...
	l1 := utf8.RuneCountInString(answer)
	l2 := utf8.RuneCountInString(guess)

	matchLen := matchLength(l1, l2)

	// m - matches, t - transpositions
	m, t := count([]rune(answer), []rune(guess), matchLen)
